...
def step_limit_for_distance(distance: float, intercept: float = 80.0, slope: float = 90.0) -> int:
        ...
        for rotation_index, rotation in enumerate(BATTERY_ROTATIONS):
            for aspect_index, aspect in enumerate(BATTERY_ASPECTS):
                ...
                tasks[name] = TaskConfig(
                    ...
                    goal_x=distance, goal_y=0.0, goal_angle=rotation,
                ...
                metadata[name] = {
                    "dist": distance, "rot": rotation, "aspect": aspect,
                    "dist_idx": distance_index, "rot_idx": rotation_index,
...
SWEEP_BASELINES = {
    "immature": DevelopmentalParams(
        ...
        gaze_switch_rate=0.20, fixation_duration_mean=4.0, target_bias=0.30,
        simultaneous_rate=0.10, sampling_rate=0.40, perceptual_noise=0.30,
        location_acuity=0.78, orientation_acuity=0.25, relation_acuity=0.10,
        wm_capacity=2, wm_decay=0.15, wm_unfixated_decay=0.34,
        affordance_coupling=0.35, affordance_noise=0.22, planning_horizon=2,
        motor_noise=0.21, habit_strength=0.72, goal_directed_strength=0.28,
        correction_rate=0.13, correction_delay=2, initiation_threshold=0.22,
    ),
    "mid": DevelopmentalParams(
        name="baseline_mid",
        gaze_switch_rate=0.35, fixation_duration_mean=3.5, target_bias=0.40,
        simultaneous_rate=0.20, sampling_rate=0.55, perceptual_noise=0.20,
        location_acuity=0.85, orientation_acuity=0.45, relation_acuity=0.20,
        wm_capacity=3, wm_decay=0.10, wm_unfixated_decay=0.25,
        affordance_coupling=0.50, affordance_noise=0.18, planning_horizon=3,
        motor_noise=0.18, habit_strength=0.50, goal_directed_strength=0.50,
        correction_rate=0.18, correction_delay=1, initiation_threshold=0.28,
    ),
    "mature": DevelopmentalParams(
        name="baseline_mature",
        gaze_switch_rate=0.48, fixation_duration_mean=2.2, target_bias=0.55,
        simultaneous_rate=0.38, sampling_rate=0.75, perceptual_noise=0.12,
        location_acuity=0.95, orientation_acuity=0.70, relation_acuity=0.42,
        wm_capacity=4, wm_decay=0.06, wm_unfixated_decay=0.14,
        affordance_coupling=0.72, affordance_noise=0.11, planning_horizon=5,
        motor_noise=0.12, habit_strength=0.25, goal_directed_strength=0.75,
        correction_rate=0.25, correction_delay=1, initiation_threshold=0.42,
    ),
}
...
SWEEP_TASKS = {
    ...
    "hard_balanced": TaskConfig(
        name="hard_balanced",
        start_x=0.0, start_y=0.0, start_angle=0.0,
        goal_x=0.90, goal_y=0.0, goal_angle=math.pi * (3 / 4),
        obj_width=0.60, obj_height=0.30,
        max_timesteps=180,
    ),
    ...
    "translation_dominant": TaskConfig(
        ...
        start_x=0.0, start_y=0.0, start_angle=0.0,
        goal_x=1.10, goal_y=0.0, goal_angle=0.20,
        obj_width=0.40, obj_height=0.40,
        max_timesteps=180,
    ),
    ...
    "rotation_dominant": TaskConfig(
        ...
        start_x=0.0, start_y=0.0, start_angle=0.0,
        goal_x=0.35, goal_y=0.0, goal_angle=math.pi * (3 / 4),
        obj_width=0.60, obj_height=0.30,
        max_timesteps=180,
    ),
}
...
PARAM_GROUPS = {
    ...
    "Perception": [
        "sampling_rate", "perceptual_noise", "location_acuity",
        "orientation_acuity", "relation_acuity",
    ],
...
MATRIX_TASKS = {
    ...
    "very_easy": TaskConfig(
        name="very_easy",
        goal_x=0.30, goal_y=0.0, goal_angle=0.53,
        obj_width=0.40, obj_height=0.40, max_timesteps=120,
    ),
    "easy": TaskConfig(
        name="easy",
        goal_x=0.35, goal_y=0.0, goal_angle=0.60,
        obj_width=0.30, obj_height=0.45, max_timesteps=120,
    ),
    "medium_rotated": TaskConfig(
        name="medium_rotated",
        goal_x=0.50, goal_y=0.0, goal_angle=1.20,
        obj_width=0.30, obj_height=0.60, max_timesteps=120,
    ),
    "very_hard": TaskConfig(
        name="very_hard",
        goal_x=0.95, goal_y=0.0, goal_angle=2.60,
        obj_width=0.60, obj_height=0.30, max_timesteps=170,
    ),
    "extreme": TaskConfig(
        name="extreme",
        goal_x=1.10, goal_y=0.0, goal_angle=math.pi,
        obj_width=0.60, obj_height=0.30, max_timesteps=190,
    ),
    # Profile D cannot be brought off ceiling by just geometry so use step limit
    "time_pressured": TaskConfig(
        name="time_pressured",
        goal_x=1.10, goal_y=0.0, goal_angle=math.pi,
        obj_width=0.60, obj_height=0.30, max_timesteps=62,
    ),
}
